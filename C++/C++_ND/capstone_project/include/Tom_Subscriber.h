...
class Tom_Subscriber : private Socket 
{
    public:
        //Constructor - Assign value [Publisher Address, Publisher Port NO, Max_Number_Client, Message_queue_len]
        Tom_Subscriber(std::string addr, std::string port_no, std::string topic_name, int max_no_client, int msg_queue_length );
    ...
    private:
        std::unique_ptr<const char> _addr; //remote address or Host Name
        std::unique_ptr<const char> _port_no; //remote Port or Host port
